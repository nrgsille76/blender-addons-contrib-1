...
def process_next_chunk(context, file, previous_chunk, importedObjects, IMAGE_SEARCH, KEYFRAME):
    ...
    while (previous_chunk.bytes_read < previous_chunk.length):
        ...
        #is it a material chunk?
            ...
            diagonal = math.copysign(math.sqrt(pow(aim[1],2)+pow(aim[0],2)),aim[1])
            hypo = math.copysign(math.sqrt(pow(diagonal,2)+pow(aim[2],2)),aim[1])
            angle = math.radians(90)-math.copysign(math.acos(diagonal/hypo), aim[2])
            contextLamp.rotation_euler[0] = -1*math.copysign(angle, aim[1])
            contextLamp.rotation_euler[2] = -1*(math.radians(90)-math.acos(aim[0]/diagonal))
            new_chunk.bytes_read += SZ_3FLOAT
            ...
            direction = math.copysign(math.sqrt(pow(cam[1],2)+pow(cam[0],2)),cam[1])
            new_chunk.bytes_read += SZ_3FLOAT
            temp_data = file.read(SZ_FLOAT)   # triangulating camera angles
            focus = math.copysign(math.sqrt(pow(direction,2)+pow(cam[2],2)), cam[1])
            pitch = math.radians(90)-math.copysign(math.acos(direction/focus), cam[2])
